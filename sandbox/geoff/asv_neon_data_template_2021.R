...
current_run <- 
  paired %>% 
  dplyr::filter(sequencerRunID == seqrun & year == 2021)
...
if(nrow(current_bact) > 0){
  ...
  saveRDS(bact_seqtab.nochim,file.path(outdir,paste0(seqrun,'_2021',"_bacteria_seqtab.RDS")))
}
...
if(nrow(current_fung) > 0){
  ...
  if(!dir.exists(outdir)){dir.create(outdir,recursive = TRUE)}
  saveRDS(fung_seqtab.nochim,file.path(outdir,paste0(seqrun,'_2021',"_fungi_seqtab.RDS")))
